...
Los requisitos trazan con los objetivos de 1.2: OE1 en RNF-01 y RNF-04; OE2 en RF-01, RF-03, RF-04, RF-08 y RF-09; OE3 en RF-05 y RNF-04; OE4 en RNF-03 y RNF-10; OE5 se aborda en 3.3 y 3.5, y en el Capítulo 7.
...
| Evasión del proxy (acceso directo a la API) | Secreto interno proxy→API (≥32 bytes) + *middleware* que bloquea las rutas de navegador a las instancias; la lista blanca de cortafuegos del puerto 8000 está documentada como refuerzo pendiente de instalar | `provision/main.py`; `nginx/install.sh` |
...
**Protección de datos.** La plataforma trata datos personales de estudiantes y queda sujeta al Reglamento General de Protección de Datos (Unión Europea, 2016) y a su desarrollo español, la LOPDGDD (España, 2018). Los datos tratados son mínimos: correo del alumno, un identificador opaco, marcas de actividad y, con finalidad exclusiva de seguridad, direcciones IP del canje de enlaces, de los accesos de administración y de las propias VMs (también dato personal); no se almacenan contraseñas (RNF-07) ni categorías especiales, y los registros excluyen secretos y enlaces de sesión, lo que materializa la minimización del artículo 5.1.c). <!-- fuente: provision/db.py (enrollments, auth_tokens.used_from_ip, admin_logins, vm_tokens.vm_ip); README.md:Identidad y seguridad --> La limitación del plazo de conservación se aplica al contenido de trabajo: las VMs y aplicaciones se destruyen por inactividad o al vencer el curso. <!-- fuente: provision/reap.py; docs/USO.md:Sesión y caducidad --> Como límite honesto, las filas históricas no se purgan automáticamente, por lo que un ejercicio del derecho de supresión exige hoy intervención manual (deuda técnica, Capítulo 9). <!-- fuente: docs/DEPLOY.md:Deudas y límites conocidos --> En una implantación real el responsable sería la institución educativa, con obligaciones organizativas (base de licitud, registro de actividades, información al alumnado y, si es administración pública, el Esquema Nacional de Seguridad) de alcance jurídico que exceden el alcance técnico de este trabajo.
...
La distribución merece una reflexión crítica honesta. No se formalizó un cronograma inicial —carencia que forma parte de la lección aprendida—; el planteamiento implícito era un avance lineal por fases, y la realidad fue intermitente: tras un arranque intenso en diciembre-enero, la actividad de febrero fue nula y la de marzo a mayo combinó documentación, refactorización y una primera iteración de implementación, por compaginar el proyecto con otras obligaciones. El grueso de la implementación se concentró en junio, coincidiendo con la adopción de la metodología de agentes descrita en el Capítulo 5, que multiplicó el ritmo de entrega respecto de las etapas anteriores; el cierre de producto se completó en julio. La lección de planificación es doble: la estimación inicial infravaloró el coste de integración y sobrevaloró la continuidad del esfuerzo disponible, y concentrar la implementación al final solo fue viable porque las fases tempranas habían fijado ya la arquitectura y sus reglas. En horas-persona, como estimación declarada a partir de la densidad del historial —no de un registro horario, que no se llevó—, el esfuerzo se aproxima a 300 horas: unas 70 en la etapa 1, 40 en la 2, 120 en la 3 y 70 en la 4; el alumno debe cotejar esta cifra con su propio criterio si dispone de un registro más preciso.
