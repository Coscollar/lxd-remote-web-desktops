...
Como se declaró en 1.4, los agentes son una herramienta bajo supervisión, no un desarrollador autónomo. Ningún artefacto se integró sin revisión del autor y validación contra el host real; los hallazgos de los críticos se aceptaron o rechazaron con criterio propio, y varias reglas que las skills codifican proceden de errores depurados a mano (sección 5.2). El efecto observable fue el salto de productividad de junio descrito en 3.7: con la arquitectura ya fijada, el ritmo de entrega aumentó de forma notable —nueve integraciones de cambios en junio, varias de gran alcance, frente a cinco entre marzo y mayo— al delegar la escritura mecánica y conservar el autor el diseño, la revisión y la validación. <!-- fuente: git log (9 commits jun vs 5 mar-may); tfg/01-introduccion.md:1.4 -->

## 5.2 Problemas encontrados y soluciones

De los problemas reales del desarrollo se seleccionan los cinco más ilustrativos, cada uno en formato síntoma → causa → solución. El resto está documentado en el repositorio como avisos operativos y deudas conocidas (uno de ellos, la activación del grupo `lxd` en la sesión, se trata en la sección 6.2 como parte del flujo del instalador). <!-- fuente: CLAUDE.md:Known gotchas; docs/DEPLOY.md:Deudas y límites conocidos -->

**Finales de línea de Windows (CRLF).** *Síntoma:* scripts que fallaban en el servidor con errores crípticos del intérprete (`$'\r': command not found`) pese a ser correctos a la vista. *Causa:* el desarrollo se realizó desde Windows, cuyos editores terminan las líneas en CRLF, mientras que `bash` espera LF. *Solución* en dos barreras: los scripts se autocomprueban al arrancar y abortan con mensaje explícito si detectan CRLF, y el instalador convierte automáticamente todos los `.sh` (con `dos2unix`) antes de cualquier otra acción, sin depender de la memoria del operador. <!-- fuente: install-all.sh (cabecera y paso 0); docs/DEPLOY.md:1 -->
