...
Confirmados los datos, el instalador encadena sin más intervención cuatro bloques: preparación (conversión CRLF→LF, *preflight*, desinstalación de cualquier instalación previa —todo arranque es limpio, lo que hace el script reejecutable por construcción— y dependencias en modo no interactivo); infraestructura (FASE 0 con la imagen base, ampliaciones no destructivas de *pool* y subred, imágenes de aplicaciones, y el tratamiento del grupo `lxd`: si no está activo en la sesión, el script termina con el código reservado 100 y reintenta la fase con el grupo activado o pide el re-inicio de sesión); servicios (orquestador con generación de secretos, Guacamole, Nginx con TLS y aislamiento); y verificación final. El detalle fase a fase está en el Anexo A3. Si una fase intermedia falla (por ejemplo, certbot ante un DNS aún no propagado), el instalador se detiene con el error visible y basta corregir la causa y reejecutar el comando, sin deshacer nada a mano. <!-- fuente: install-all.sh (pasos 0-6, exit 100); README.md:Instalación; docs/DEPLOY.md:0b -->
