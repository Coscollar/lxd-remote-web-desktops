...
*Figura 4.1. Arquitectura general de la plataforma: bloques y rutas de enrutado. Los nombres `/verify`, `/verify/app` y `/admin/verify` identifican los endpoints de `provision-api` consultados por `auth_request`; en la configuración de Nginx las *locations* internas que los invocan se llaman `/verify`, `/verify_app` y `/verify_admin` respectivamente. Elaboración propia a partir de la topología documentada del proyecto.* <!-- fuente: README.md:Arquitectura; nginx/lab.conf -->
...
| provision-api | 0.0.0.0:8000 | Nginx (127.0.0.1), VMs (10.50.20.0/24) y aplicaciones (10.50.10.0/23); control por *middleware* de rutas y testigos ligados a IP (lista blanca iptables del puerto pendiente, sección 5.2) | — |
...
*Figura 4.2. Diagrama de despliegue (representado con notación de diagrama de flujo equivalente al UML de despliegue): servicios, puntos de escucha, redes internas y pools de almacenamiento sobre el host único. Elaboración propia.* <!-- fuente: README.md:Arquitectura; lxd-preseed.yaml; guacamole/docker-compose.yml; systemd/ -->

Como muestra la Figura 4.2, la comunicación es deliberadamente asimétrica: el orquestador gobierna las instancias a través de la CLI de LXD, y las VMs solo pueden responder con señales acotadas (latidos, peticiones de guardado o reinicio) hacia el puerto 8000, cuyo acceso se controla con testigos ligados a IP y con el *middleware* de rutas —la lista blanca de cortafuegos de ese puerto sigue pendiente de instalar, como se narra en la sección 5.2—; las aplicaciones no emiten hoy latido propio (véase 4.2.3) y ninguna instancia ejecuta órdenes de virtualización. <!-- fuente: README.md:Identidad y seguridad; provision/main.py (middleware); install-all.sh (sin reglas INPUT 8000) -->
...
*Figura 4.3. Diagrama de componentes del orquestador (representado con notación de diagrama de flujo equivalente al UML de componentes): módulos de `provision/` y sus dependencias. Elaboración propia a partir del código fuente.* <!-- fuente: provision/*.py -->
...
La pila se apoya deliberadamente en tecnologías estándar del ecosistema Linux (Nginx, systemd, iptables, SQLite), próximas al contenido habitual de una titulación de informática (relación detallada en 8.2); el aprendizaje real se concentró en tres piezas menos convencionales —proyectos, perfiles y *pools* de LXD; el contrato de renderizado de cloud-init; y la integración de Guacamole con una base de conexiones gestionada desde código propio—, coherente con la concentración de esfuerzo del plan de trabajo (3.7). <!-- fuente: plan de trabajo de la sección 3.7 (concentración del esfuerzo en la etapa de implementación) -->
...
[^1]: OpenZFS — https://openzfs.org/ [consulta: 15 de julio de 2026].
[^2]: uvicorn, servidor ASGI para Python — https://www.uvicorn.org/ [consulta: 15 de julio de 2026].
[^3]: Jinja2, motor de plantillas para Python — https://jinja.palletsprojects.com/ [consulta: 15 de julio de 2026].
[^4]: certbot (EFF) — https://certbot.eff.org/ ; Let's Encrypt — https://letsencrypt.org/ [consulta: 15 de julio de 2026].
[^5]: MATE Desktop Environment — https://mate-desktop.org/ [consulta: 15 de julio de 2026].
