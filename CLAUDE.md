...
- **`provision/`** — the FastAPI orchestrator, the core of the system:
  ...
  - `apps.py` — stateless app catalog + launch/reset/status endpoints (FASE 6). Student endpoints are registered twice: `/apps/*` (legacy/internal) **and** `/api/apps/*` (what the browser dashboard calls — in Nginx, `/apps/{id}/...` proxies to the app container, so the student API must travel under `location /api/`). Admin catalog CRUD: `POST/PATCH/DELETE /admin/apps/{id}` (DELETE = soft-delete + enqueued destruction of live instances via `destroy_app` jobs, never a synchronous loop).
  - `admin.py` — admin console backend (F2): labs CRUD, enrollments CRUD (keyset pagination by rowid), `POST /admin/instances/{nombre}/destroy?tipo=vm|app` (anti-TOCTOU `BEGIN IMMEDIATE`, state-guarded final UPDATE), `POST /admin/instances/launch` (same `jobs.start_launch` path as `/lab/start` — zero new launch logic). Every mutation requires `X-Requested-With: XMLHttpRequest`. Admin account management is deliberately SQL-only (no UI/API endpoint).
  - `web.py` — Jinja2 HTML routes for students and admin (login, dashboard, console); `web/templates/`, `web/static/js/{login,dashboard,admin}.js`. All JS is static (the CSP has no `unsafe-inline`/nonce); DOM rendering is `createElement`/`textContent` only, never interpolated `innerHTML`.
...
- **`cloud-init-template.yml`** — Jinja2 template (not native `${}` cloud-init syntax) rendered per-student by `provision-api` and piped via **stdin** to `lxc launch -c user.user-data=-` (never as a shell argument — avoids flag injection). Full contract in `docs/DEPLOY.md` (Anexo A). Never commit a rendered YAML (it contains the live VM service token) — it's render-in-memory only.
...
- **`install-all.sh` / `uninstall-all.sh`** — single entrypoints for full install/uninstall (including the FASE 6 apps portal). Run without flags on a TTY, `install-all.sh` enters a **guided wizard** (asks domain, certbot email, first console admin — seeded into `admins` — and SMTP credentials with hidden password; validates each against closed-charset regexes, shows a summary, asks confirmation); with `--domain`/`--email` flags it stays non-interactive for automation. It starts with a fail-fast **preflight** (OS 22.04/24.04, `/dev/kvm` ABORT, zfs.ko ABORT with auto-install attempt, snapd + `snap wait`, ports 80/443, DNS/ufw/RAM/disk warnings; `--skip-preflight` degrades aborts to warnings), exports `DEBIAN_FRONTEND=noninteractive` + debconf-preseeds iptables-persistent, and never prints generated secrets to stdout (they live in `/etc/provision/provision.env`). `nginx/install.sh` generates `/etc/nginx/conf.d/lab-internal.conf` (the `$internal_token` map) from that env file and fails if the token is missing/short.
...
- **TFG (memoria académica, ETSINF-UPV)** — orchestrators `tfg-plan` (outline mapped to repo evidence) and `tfg-build` (writes chapters into `tfg/`, one .md per chapter); writers `tfg-estado-arte` (only agent allowed to research the web, real ISO 690 references), `tfg-analista` (requirements/security/legal/risks/budget), `tfg-disenador` (architecture/detailed design/tech), `tfg-desarrollo-doc` (development/deployment/tests narrative); critics `critic-tfg-formato` (official ETSINF structure), `critic-tfg-academico` (tribunal-grade style/rigor), `critic-tfg-veracidad` (every claim cross-checked against the repo). Skills: `tfg-formato-etsinf` (official ETSINF structure & format rules), `tfg-estilo-academico`, `tfg-ciclo-vida` (lifecycle→repo-evidence→chapter map). Rule: nothing invented — student data stays `[RELLENAR]`, unverified content gets `[ELABORAR]`/`[PENDIENTE]`, and every technical claim carries a `<!-- fuente: ... -->` comment.
- **`DOIN.md`, `Entorno de Laboratorio con LXD.md`** — design notes, not executable (and required reading for the `.claude/agents/` subagents — do not delete). If they conflict with the actual scripts, trust the scripts.
